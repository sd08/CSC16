         ...
         if True:
            if newurl not in visited:
               ...
               try:
                  ...
                  for link1 in links1:
                     newurl1=link1
                     host=urlparse.urlparse(newurl1).hostname
                     if True:
                        if newurl1 not in visited:
                           count2+=1
                           visited.append(newurl1)
                           print host+' '+u+' link '+str(count1)+' '+str(count2)
                           directory=tempdir+'link '+str(count1)+'/link '+str(count1)+' '+str(count2)+'/'
                           if not os.path.exists(directory):
                              os.mkdir(directory)
                           f = open(directory+'link '+str(count1)+' '+str(count2)+'.txt', 'a')
                           ips = socket.gethostbyname(host)
                           #ns=get_authoritative_nameserver(host)
                           writeIP(f,ips,host)
                           f.close()
                           ...
                           try:
                              ...
                              for iii in br.links():
                                 tempcount2+=1
                                 if tempcount2<(limit2+1):
                                    links2.append(urlparse.urljoin(iii.base_url,iii.url))
                                 else:
                                    break
                              for link2 in links2:
                                 newurl2=link2
                                 host=urlparse.urlparse(newurl2).hostname
                                 if True:
                                    if newurl2 not in visited:
                                       count3+=1
                                       visited.append(newurl2)
                                       print host+' '+u+' link '+str(count1)+' '+str(count2)+' '+str(count3)
                                       directory=tempdir+'link '+str(count1)+'/link '+str(count1)+' '+str(count2)+'/link '+str(count1)+' '+str(count2)+' '+str(count3)+'/'
                                       if not os.path.exists(directory):
                                          os.mkdir(directory)
                                       f = open(directory+'link '+str(count1)+' '+str(count2)+' '+str(count3)+'.txt', 'a')
                                       ips = socket.gethostbyname(host)
                                       #ns=get_authoritative_nameserver(host)
                                       writeIP(f,ips,host)
                                       f.close()
...
print str(count)+' '+str(count1)+' '+str(count2)+' '+str(count3)
